...
print(find_nth_prime_simple(10001))
```

### Optimized Solution
```python
def find_nth_prime_optimized(n):
    # First prime numbers for optimization
    primes = [2]
    num = 3
    
    # Using wheel factorization - checking only odd numbers
    while len(primes) < n:
        is_prime = True
        sqrt = int(num ** 0.5)
        
        # Check divisibility only by found primes up to sqrt
        for prime in primes:
            if prime > sqrt:
                break
            if num % prime == 0:
...
The optimized solution is more efficient because it:
- Only checks odd numbers after 2
- Stores previously found primes
- Only checks divisibility by known primes
- Only checks up to square root
- Uses wheel factorization principle
- Reduces number of operations significantly

Both solutions will give the same result, but the optimized version will run much faster, especially for finding large prime numbers.
...
1. **Understanding the Problem**
- We have a 1000-digit number
- Need to find 13 consecutive digits within this number
- Must calculate product of these 13 digits
- Need to find the largest such product possible

2. **Example Given**
- Shows 4 adjacent digits: 9,9,8,9
- Their product is 5832
- Demonstrates how to calculate product of adjacent digits

3. **Expected Output**
- A single number representing the largest product
- Must be calculated from exactly 13 consecutive digits
- Must be the maximum possible such product
